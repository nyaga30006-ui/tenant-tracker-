...
export function MoveInTenantDialog({ onClose, onSaved, room }: MoveInTenantDialogProps) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    onSaved({
      depositHeld: numericDepositHeld,
  ...
  return (
    <Modal description={`Create a new occupancy for ${room.number}. Previous tenants and payments will remain in the room history.`} onClose={onClose} title="Move tenant in">
      <form className="modal-form two-grid" onSubmit={submit}>
        ...
        <label className="field">Monthly rent (KES)<input inputMode="numeric" min="1" onChange={(event) => setRent(event.target.value)} required type="number" value={rent} /></label>
        <label className="field">Deposit required (KES)<input inputMode="numeric" min="0" onChange={(event) => setDepositRequired(event.target.value)} required type="number" value={depositRequired} /></label>
        <label className="field field--wide">Deposit already held (KES)<input inputMode="numeric" max={numericDepositRequired || undefined} min="0" onChange={(event) => setDepositHeld(event.target.value)} placeholder="0" type="number" value={depositHeld} /><small className="field-help">Use this only for money received before creating the occupancy. New deposit payments should be recorded normally.</small></label>
        <label className="choice-field info-box field--wide"><input checked={electricityDueEnabled} onChange={(event) => setElectricityDueEnabled(event.target.checked)} type="checkbox" /><span><strong>Add electricity to the monthly room charge</strong><small>The configured electricity fee will be included in the tenant’s recurring balance.</small></span></label>
