...
export function useLifetimeCollectionMetrics() {
  ...
  return useMemo(() => {
    ...
    // Confirmed collections + the live net balance reconstructs the total
    // charged since tracking began without counting carried arrears twice.
    ...
    return {
      collected,
      credits,
      depositAppliedToBalances,
      dueAndArrears,
      paymentCount: confirmedCollections.length,
      rate,
      totalCharged,
    };
  }, [payments, rooms, tenantResidencies]);
}
