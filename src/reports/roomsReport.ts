...
export function downloadRoomsReport(property: Property, rooms: Room[], filters: string[] = []): string {
  ...
  return downloadTableReport({
    columns: ["Room", "Tenant", "Floor", "Rent", "Paid", "Arrears", "Deposit", "Balance", "Status"],
    filename: "rooms-report",
    filters,
    propertyAddress: `${property.address}, ${property.city}`,
    propertyName: property.name,
    rows: rooms.map((room) => [room.number, room.tenant || "Vacant", room.floor === 0 ? "Ground" : `Floor ${room.floor}`, reportMoney(room.rent), reportMoney(room.paid), reportMoney(room.arrears), reportMoney(room.depositPaid ?? 0), reportMoney(roomBalance(room)), calculatedRoomStatus(room)]),
